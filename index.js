...
exports.Tlsh = exports.hist_diff3_add = exports.hist_diff2_add = exports.hist_diff1_add = void 0;
const utils_1 = require("./utils");
exports.hist_diff1_add = 1;
exports.hist_diff2_add = 2;
exports.hist_diff3_add = 6;
/*
...
class Tlsh {
    ...
    finale(str, length) {
        ...
        this.Lvalue = (0, utils_1.l_capturing)(this.data_len);
    ...
    hash() {
        ...
        for (let k = 0; k < TLSH_CHECKSUM_LEN; k++) {
            tmp.checksum[k] = (0, utils_1.swap_byte)(this.checksum[k]);
            debug && console.log("After swap_byte for checksum: tmp.checksum:" + tmp.checksum[k] + ", tlsh.checksum:" + this.checksum[k]);
        }
        tmp.Lvalue = (0, utils_1.swap_byte)(this.Lvalue);
        tmp.Q = (0, utils_1.swap_byte)(this.Q);
        ...
        this.lsh_code = (0, utils_1.to_hex)(tmp.checksum, TLSH_CHECKSUM_LEN);
        let tmpArray = new Uint8Array(1);
        tmpArray[0] = tmp.Lvalue;
        this.lsh_code = this.lsh_code.concat((0, utils_1.to_hex)(tmpArray, 1));
        tmpArray[0] = tmp.Q;
        this.lsh_code = this.lsh_code.concat((0, utils_1.to_hex)(tmpArray, 1));
        this.lsh_code = this.lsh_code.concat((0, utils_1.to_hex)(tmp.tmp_code, CODE_SIZE));
    ...
    totalDiff(other, len_diff = false) {
        if (this == other) {
            return 0;
        }
        ...
        let diff = 0;
        if (len_diff) {
            let ldiff = (0, utils_1.mod_diff)(this.Lvalue, other.Lvalue, RANGE_LVALUE);
            if (ldiff == 0)
                diff = 0;
            else if (ldiff == 1)
                diff = 1;
        ...
        let q1diff = (0, utils_1.mod_diff)(getQLo(this.Q), getQLo(other.Q), RANGE_QRATIO);
        if (q1diff <= 1)
            diff += q1diff;
        else
            diff += (q1diff - 1) * 12;
        let q2diff = (0, utils_1.mod_diff)(getQHi(this.Q), getQHi(other.Q), RANGE_QRATIO);
        if (q2diff <= 1)
            diff += q2diff;
        else
            diff += (q2diff - 1) * 12;
        ...
        diff += (0, utils_1.h_distance)(CODE_SIZE, this.tmp_code, other.tmp_code);
        return diff;
    }
    find_quartile(quartiles) {
        let buf = {
    ...
    fromTlshStr(str) {
        ...
        let tmp = (0, utils_1.from_hex)(str);
        // Order of assignment is based on order of fields in lsh_bin
        // Also note that TLSH_CHECKSUM_LEN is 1
        let i = 0;
        this.checksum[i] = (0, utils_1.swap_byte)(tmp[i++]);
        this.Lvalue = (0, utils_1.swap_byte)(tmp[i++]);
        this.Q = (0, utils_1.swap_byte)(tmp[i++]);
